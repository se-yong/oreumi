...
from sklearn.model_selection import train_test_split # 학습과 테스트 분리(학습한 것을 기반으로 테스트하면 안되기 때문) 
...
model = KNeighborsClassifier(n_neighbors=1) # 옆에 이웃이 하나만 존제
...
print("테스트 세트 정확도: {:.2f}".format(np.mean(y_p == y_test)))
